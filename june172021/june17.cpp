...
int pkey_rsa_dup(EVP_PKEY *dst_pkey, EVP_PKEY *src_key, int key_type) {
    // Validate underlying key type - Only allow a RSA key

    RSA *rsa = EVP_PKEY_get1_RSA(src_key); // Get the underlying RSA key
    dst_pkey = EVP_PKEY_new();

    if(key_type == 0) {
	EVP_PKEY_assign_RSA(dst_pkey, RSAPrivateKey_dup(rsa));
    // EVP_PKEY_set1_RSA also adjusts the other members in dst_pkey
    } else {
	EVP_PKEY_assign_RSA(dst_pkey, RSAPublicKey_dup(rsa));
    }
    return 0;
}
	...
	void **new_pri_key2)
{
	...
	std::string test = to_PEM_private(dst_pkey);
...
std::string decryptMessage(std::string cipher_text, shim_ctx_ptr_t ctx)
{
	std::string decrypt_text;
	RSA *rsa = RSA_new();
	BIO *keybio;
	keybio = BIO_new_mem_buf((unsigned char *)privatekeytoreturn.c_str(), -1);
 
	rsa = PEM_read_bio_RSAPrivateKey(keybio, &rsa, NULL, NULL);
	if (rsa == nullptr) {
		 unsigned long err = 55; //Get the error number
		char err_msg[3072] = { 0 };
		return "NULL PTR Failure";
	}
...
std::string createKeys(shim_ctx_ptr_t ctx)
{
	...
	std::string publickeytoreturn = sgx_create_rsa_pub1_key_test(RSA_MOD_SIZE, sizeof(p_e), p_n, (unsigned char*)&p_e, &public_key_test);
	...
	privatekeytoreturn = sgx_create_rsa_priv2_key_test(RSA_MOD_SIZE, sizeof(p_e), (unsigned char*)&p_e, p_p, p_q, p_dmp1, p_dmq1, p_iqmp, &private_key_test);
	...
	//string s = "";
	//std::string base64_pk = base64_encode((const unsigned char*)pout_data, pout_len);
	/*
	for(int c=0; c<384; c++){
    ...
    shim_ctx_ptr_t ctx)
{
	...
	result = decryptMessage_base64(encryptedmessage, ctx);
    }
