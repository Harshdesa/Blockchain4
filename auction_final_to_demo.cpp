...
std::string X509_to_PEM(EVP_PKEY *pkey) {
...
std::string sgx_create_rsa_pub1_key_test(int mod_size, int exp_size, const unsigned char *le_n, const unsigned char *le_e, void **new_pub_key1)
{
	...
	std::string test = to_PEM(rsa_key);
...
std::string sgx_create_rsa_priv2_key_test(int mod_size, int exp_size, const unsigned char *p_rsa_key_e, const unsigned char *p_rsa_key_p, const unsigned char *p_rsa_key_q,
...
std::string testPrintingKey(shim_ctx_ptr_t ctx)
{
	...
	return "sgx_ecc256_open_context not successful";
	}
	...
	return "sgx_ecc256_create_key_pair not successful";
	}
...
std::string testPrintingRsaKey(shim_ctx_ptr_t ctx)
{
	void *public_key = NULL;
	void *private_key = NULL;
	void *public_key_test = NULL;
	void *private_key_test = NULL;
	...
	std::string testtoreturn = sgx_create_rsa_pub1_key_test(RSA_MOD_SIZE, sizeof(p_e), p_n, (unsigned char*)&p_e, &public_key_test);

	std::string testtoreturnprivate = sgx_create_rsa_priv2_key_test(RSA_MOD_SIZE, sizeof(p_e), (unsigned char*)&p_e, p_p, p_q, p_dmp1, p_dmq1, p_iqmp, &private_key_test);
	...
	std::string base64_pk = base64_encode((const unsigned char*)public_key, 384);
	...
	s = s + "hello" + testtoreturn + "hello" + testtoreturnprivate;
...
std::string testRsaEncryption(std::string base64_pk, std::string base64_privkey, shim_ctx_ptr_t ctx)
{
	std::string privkey = base64_decode(base64_privkey);
        void* private_key = static_cast<void*>(&privkey);

	std::string pubkey = base64_decode(base64_pk);
	void* public_key = static_cast<void*>(&pubkey);
    ...
    shim_ctx_ptr_t ctx)
{
    ...
    else if (function_name == "testPrintingKey")
    {
	result = testPrintingKey(ctx);
    }
    else if (function_name == "testPrintingRsaKey")
    {
	result = testPrintingRsaKey(ctx);
    }
    else if (function_name == "testRsaEncryption")
    {
	std::string pkey = params[0];
        std::string privkey = params[1];
	result = testRsaEncryption(pkey, privkey, ctx);
    }
    else
    {
        // unknown function
